...
const ReturnFlights = () => {
  ...
  },
  {
    ...
    for(let i=0; i < numberOfRowsF ; i++){
      const row = [];
        for(let j=1; j<=6;j++){
          ...
          else if(contains((i*10+j),BookedSeatsIDs) && j%2 != 1 && temp > 0){
    ...
    for(let i=0; i < numberOfRowsB ; i++){
      const row = [];
        for(let j=1; j<=6;j++){
          if(j%2 != 0){
            row.push({id:((numberOfRowsF+i)*10+j), number: j, isReserved: true , orientation:"east"});
          }
          else{
            row.push({id:((numberOfRowsF+i)*10+j), number: j, isReserved: true, orientation:"west"});
            row.push(null)
          }
        }
        rows.push(row);
    ...
    for(let i=0; i < numberOfRowsE ; i++){
      const row = [];
        for(let j=1; j<=6;j++){
          if(j%2 != 0){
            row.push({id:((numberOfRowsF+numberOfRowsB+i)*10+j), number: j, isReserved: true, orientation:"east"});
          }
          else{
            row.push({id:(i*10+j), number: j, isReserved: true, orientation:"west"});
            row.push(null)
    ...
    for(let i=0; i < numberOfRowsB ; i++){
      const row = [];
        for(let j=1; j<=6;j++){
          if(contains(((numberOfRowsF+i)*10+j),BookedSeatsIDs) && j%2 != 0 && temp > 0){
            row.push({id:((numberOfRowsF+i)*10+j), number: j, isReserved: true , orientation:"east"});
          }
          else if(contains(((numberOfRowsF+i)*10+j),BookedSeatsIDs) && j%2 != 1 && temp > 0){
            row.push({id:((numberOfRowsF+i)*10+j), number: j, isReserved: true, orientation:"west"});
          }
          else if(j%2 != 0 && temp > 0){
            row.push({id:((numberOfRowsF+i)*10+j), number: j, isSelected: false , orientation:"east"});
          }
          else if(temp>0){
            row.push({id:((numberOfRowsF+i)*10+j), number: j, isSelected: false, orientation:"west"});
            row.push(null)
          }
          temp--;
        }
      ...
      const row = [];
        for(let j=1; j<=6;j++){
          if(j%2 != 0){
            ...
            row.push({id:((numberOfRowsF+numberOfRowsB+i)*10+j), number: j, isReserved: true, orientation:"west"});
            row.push(null)
          }
        }
        rows.push(row);
    ...
    for(let i=0; i < numberOfRowsE ; i++){
      const row = [];
        for(let j=1; j<=6;j++){
          if(contains(((numberOfRowsF+numberOfRowsB+i)*10+j),BookedSeatsIDs) && j%2 != 0 && temp > 0){
            row.push({id:(i*10+j), number: j, isReserved: true , orientation:"east"});
          }
          else if(contains(((numberOfRowsF+numberOfRowsB+i)*10+j),BookedSeatsIDs) && j%2 != 1 && temp > 0){
            row.push({id:((numberOfRowsF+numberOfRowsB+i)*10+j), number: j, isReserved: true, orientation:"west"});
          }
          else if(j%2 != 0 && temp > 0){
            row.push({id:((numberOfRowsF+numberOfRowsB+i)*10+j), number: j, isSelected: false, orientation:"east"});
          }
          else if(temp>0){
            row.push({id:((numberOfRowsF+numberOfRowsB+i)*10+j), number: j, isSelected: false, orientation:"west"});
            row.push(null)
          }
          temp--;
        }
  ...
  const handleConfirmSeats = () => {
    const flight = {TakenSeats:takenSeats , Flight_id:localStorage.getItem("SelectedFlightChooseSeats") , User_ID:1 , FirstClassSeats:FirstClassSeats , BusinessClassSeats:BusinessClassSeats , EconomySeats:EconomyClassSeats }  
    if(localStorage.getItem("SelectedFlightChooseSeats") == localStorage.getItem("FlightIDAro")){
